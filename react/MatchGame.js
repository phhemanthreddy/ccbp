...
class App extends Component {
  ...
  componentDidMount() {
    this.timerStop = setInterval(() => this.timer(), 1000)
  }

  clickTab = b => {
    this.setState({activeTab: b.target.id})
  }

  clickPic = id => {
    const {finishedList, score, url} = this.state
    console.log(finishedList.includes(id))
    ...
    if (finishedList.includes(id)) {
      ...
      if (url.id === id) {
        this.setState(p => ({
          score: p.score + 1,
          finishedList: [...p.finishedList, id],
          url: imagesList
            .filter(e => !finishedList.includes(e.id))
  ...
  timer = () => {
    ...
    if (seconds) {
      this.setState(p => ({seconds: p.seconds - 1}))
    } else {
      clearInterval(this.timerStop)
      this.setState({gameOver: true})
    }
  ...
  render() {
    const {seconds, activeTab, finishedList, gameOver, score, url} = this.state
    ...
    return (
      <div>
        ...
        {gameOver ? (
          ...
          <div>
            <img src={url.imageUrl} alt="match" style={{width: '10%'}} />

            <button type="button" id="Fruits" onClick={this.clickTab}>
              Fruits
            </button>
            <button type="button" id="Animals" onClick={this.clickTab}>
              Animals
            </button>
            <button type="button" id="Places" onClick={this.clickTab}>
              Places
            </button>
            <div>
              {activeTab === 'Fruits' &&
